...
import {
  ...
  updateProjectMembershipFailed,
  updateProjectMembershipRequested,
  updateProjectMembershipSucceeded,
} from '../../../actions';
import api from '../../../api';
...
export function* updateProjectMembershipRequest(id, data) {
  yield put(updateProjectMembershipRequested(id, data));

  try {
    const { item } = yield call(request, api.updateProjectMembership, id, data);

    const action = updateProjectMembershipSucceeded(item);
    yield put(action);
    ...
    const action = updateProjectMembershipFailed(id, error);
    yield put(action);
