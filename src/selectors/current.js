...
export const cardsForCurrentUserSelector = createSelector(
  orm,
  (state) => currentUserIdSelector(state),
  ({ User }, id) => {
    if (!id) {
    ...
    const cardsArrayBumpy = userModel
      ...
      .map(({ project: projectModel }) => {
        ...
        projectModel.boards.toModelArray().forEach((boardModel) => {
          ...
          // console.log('boardModel.name = ', boardModel.name);

          const cardsRefArray = boardModel.cards
            .toRefArray();
            // .toModelArray();

          // console.log('cardsRefArray = ', cardsRefArray);
      ...
      // console.log('cardsArrayBumpy = ', cardsArrayBumpy);

      const cardsArrayFlat = cardsArrayBumpy.flat();

      // console.log('cardsArrayFlat = ', cardsArrayFlat);

      /*
      // // For dev & debug, let's set the status manually
      const cardsArrayWithStatus = cardsArrayFlat.map(caf => {
        return {
          ...caf,
          status: 1,
        }
      ...
      console.log('cardsArrayWithStatus = ', cardsArrayWithStatus);

      return cardsArrayWithStatus;
      */
